...
public class TransactionService {
	...
	public String deposit(String userAccount, BigDecimal amount) {
		...
		virtualAccount = (VirtualAccount) session.get(VirtualAccount.class,
				userAccount);
		session.close();
		...
		if (virtualAccount == null) {
			...
			if (amount.doubleValue() > 0) {
				...
				virtualAccount.setBalance(virtualAccount.getBalance().add(
						amount));
				...
				message = new FacesMessage("Deposit Success");
				message.setSeverity(FacesMessage.SEVERITY_INFO);
				FacesContext.getCurrentInstance().addMessage(null, message);
	...
	public String withdrawl(String accountNo, BigDecimal amount) {
		Session session = HibernateUtil.openSession();
		Transaction trx = session.beginTransaction();
		VirtualAccount virtualAccount = (VirtualAccount) session.get(
				VirtualAccount.class, accountNo);
		session.close();
		if (virtualAccount == null) {
			...
			if (virtualAccount.getBalance().doubleValue() > amount
					.doubleValue() && amount.doubleValue() > 0) { // Check
																	// Drawing
				session = HibernateUtil.openSession();
				trx = session.beginTransaction();

				virtualAccount.setBalance(new BigDecimal(virtualAccount
						.getBalance().doubleValue() - amount.doubleValue()));
	...
	public void transfer(String accountNo, String targetAccountNo,BigDecimal amount) {
		...
		VirtualAccount account = (VirtualAccount) session.get(
				VirtualAccount.class, accountNo);
		VirtualAccount virtualAccount = (VirtualAccount) session.get(
				VirtualAccount.class, targetAccountNo);
		if (account.getUserProfile().getUserType().equals('C')
				&& virtualAccount.getUserProfile().getUserType().equals('C')) {

			if (account.getBalance().doubleValue() > amount.doubleValue()) {

				account.setBalance(new BigDecimal(account.getBalance()
	...
	public void purchase(String nameProduct, String accountNo) {
		...
		if (virtualAccount.getUserProfile().getUserType().equals('C')) {
			bayar = product.getPrice().doubleValue()
					+ product.getMargin().doubleValue();
			virtualAccount.setBalance(new BigDecimal(virtualAccount
					.getBalance().doubleValue() - bayar));
		} else if (virtualAccount.getUserProfile().getUserType().equals('M')) {
			bayar = product.getPrice().doubleValue()
					+ (product.getMargin().multiply(
							product.getSharePercentage()).doubleValue());
			virtualAccount.setBalance(new BigDecimal(virtualAccount
					.getBalance().doubleValue() - bayar));
